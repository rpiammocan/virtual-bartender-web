...
export default function RecipeManagePage({ onHome, onBack, openRecipe }: Props) {
  ...
  const [manualIngredients,setManualIngredients]=useState<ManualIngredient[]>([{quantity:"1",unit:"oz",name:""}]);
  ...
  function updateManualIngredient(index:number,key:keyof ManualIngredient,value:string){setManualIngredients(current=>current.map((item,i)=>i===index?{...item,[key]:value}:item));}
  function addManualIngredient(){setManualIngredients(current=>[...current,{quantity:"1",unit:"oz",name:""}]);}
  function removeManualIngredient(index:number){setManualIngredients(current=>current.filter((_,i)=>i!==index));}
  function saveManual(){
    setError("");setSavedKey(null);
    const valid=manualIngredients.filter(item=>item.name.trim());
    if(!manualName.trim()||valid.length===0){setError("Enter a recipe name and at least one ingredient.");return;}
    try{
      const recipe=createCustomRecipe({
        ...
        source:"Manual Entry",
        ingredients:valid.map(item=>ingredientFromName(item.name,Number(item.quantity)||1,item.unit||"pc")),
      });
      saveCustomRecipe(recipe);
      setSavedKey(recipe.key);
      setManualName("");setManualType("cocktail");setManualDescription("");setManualInstructions("");setManualIngredients([{quantity:"1",unit:"oz",name:""}]);
    }catch(err){setError(err instanceof Error?err.message:"Unable to save recipe.");}
  }

  return <main className="page theme-recipes">
    <header className="app-header"><button className="back-button" onClick={onHome}>← Home</button><button className="back-button" onClick={onBack}>← Recipes</button><div className="page-heading"><span className="page-heading-icon">📖</span><h1>Add / Import Recipe</h1></div></header>
    ...
    <div className="toolbar"><button className={mode==="manual"?"primary":""} onClick={()=>setMode("manual")}>Manual Add</button><button className={mode==="single"?"primary":""} onClick={()=>setMode("single")}>Import One Recipe</button><button className={mode==="bulk"?"primary":""} onClick={()=>setMode("bulk")}>Bulk Import from Website</button></div>
    <p className="lede">{mode==="manual"?"Type in your own recipe and save it directly in this browser.":mode==="single"?"Paste one recipe URL. Review and edit everything before saving it locally.":"Paste a recipe collection page. Scan it, select recipes, and save them to your local Virtual Bartender browser database."}</p>

    {mode!=="manual"&&<div className="toolbar"><input className="wide-input" placeholder="https://..." value={url} onChange={e=>setUrl(e.target.value)}/><button className="primary" disabled={busy||!url.trim()} onClick={()=>void(mode==="single"?importRecipe():scan())}>{busy?"Working…":mode==="single"?"Import & Review":"Scan Website"}</button></div>}
    {error&&<p className="error">{error}</p>}

    {mode==="manual"&&<section className="detail-card"><p className="eyebrow">Manual recipe entry</p><label>Recipe name<input className="wide-input" value={manualName} onChange={e=>setManualName(e.target.value)} placeholder="Recipe name"/></label><label> Type <select value={manualType} onChange={e=>setManualType(e.target.value as "cocktail"|"mocktail")}><option value="cocktail">Cocktail</option><option value="mocktail">Mocktail</option></select></label><label>Description<textarea rows={3} value={manualDescription} onChange={e=>setManualDescription(e.target.value)} placeholder="Optional description"/></label><h3>Ingredients</h3><div className="import-ingredients">{manualIngredients.map((item,index)=><div className="import-row" key={index}><input value={item.quantity} onChange={e=>updateManualIngredient(index,"quantity",e.target.value)} placeholder="Qty"/><select value={item.unit} onChange={e=>updateManualIngredient(index,"unit",e.target.value)}><option value="oz">oz</option><option value="ml">ml</option><option value="tsp">tsp</option><option value="tbsp">tbsp</option><option value="dash">dash</option><option value="pc">pc</option><option value="cup">cup</option></select><input value={item.name} onChange={e=>updateManualIngredient(index,"name",e.target.value)} placeholder="Ingredient"/><button className="danger-link" type="button" onClick={()=>removeManualIngredient(index)}>Remove</button></div>)}</div><div className="toolbar"><button type="button" onClick={addManualIngredient}>+ Ingredient</button></div><h3>Instructions</h3><textarea rows={8} value={manualInstructions} onChange={e=>setManualInstructions(e.target.value)} placeholder="Mixing instructions"/><div className="toolbar"><button className="primary" onClick={saveManual}>Save Recipe</button></div>{savedKey&&<p className="success">Recipe saved successfully. <button className="link-button" onClick={()=>openRecipe(savedKey)}>View Recipe</button></p>}</section>}
