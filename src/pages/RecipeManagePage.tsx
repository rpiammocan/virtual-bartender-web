...
const INGREDIENT_NAMES = BUILTIN_INGREDIENTS.map((ingredient) => ingredient.name);
...
export default function RecipeManagePage({ onHome, onBack, openRecipe }: Props) {
  ...
  function updateIngredient(index: number, key: "quantity" | "unit" | "name", value: number | string | null) {
    setDraft((current) => {
  ...
  function saveDraft(item: ImportDraft) {
    const recipe = createCustomRecipe({
      ...
      ingredients: item.ingredients
        ...
        .map((ingredient) => ingredientFromName(ingredient.name, ingredient.quantity ?? 1, ingredient.unit ?? "pc")),
    });
  ...
  function updateManualIngredient(index: number, key: keyof ManualIngredient, value: string) {
    setManualIngredients((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item));
  }
  ...
  return (
    <main className="page theme-recipes">
      <header className="app-header">
        ...
        <div className="page-heading"><span className="page-heading-icon">📖</span><h1>Add / Import Recipe</h1></div>
      </header>
      ...
      <p className="lede">{mode === "manual" ? "Enter the same recipe details you would review after an import, but type them in yourself." : mode === "single" ? "Paste one recipe URL. Review and edit everything before saving it locally." : "Paste a recipe collection page. Scan it, select recipes, and save them to your local Virtual Bartender browser database."}</p>

      {mode !== "manual" && <div className="toolbar"><input className="wide-input" placeholder="https://..." value={url} onChange={(event) => setUrl(event.target.value)} /><button className="primary" disabled={busy || !url.trim()} onClick={() => { if (mode === "single") void importRecipe(); else void scan(); }}>{busy ? "Working…" : mode === "single" ? "Import & Review" : "Scan Website"}</button></div>}
      {error && <p className="error">{error}</p>}

      {mode === "manual" && <section className="detail-card">
        <p className="eyebrow">Add Recipe Manually</p>
        <label>Recipe name<input className="wide-input" value={manualName} onChange={(event) => setManualName(event.target.value)} placeholder="Recipe name" /></label>
        <label>Type <select value={manualType} onChange={(event) => setManualType(event.target.value as "cocktail" | "mocktail")}><option value="cocktail">Cocktail</option><option value="mocktail">Mocktail</option></select></label>
        <h3>Ingredients</h3>
        <div className="import-ingredients">
          {manualIngredients.map((item, index) => <div className="import-row" key={index}>
            <input value={item.quantity} onChange={(event) => updateManualIngredient(index, "quantity", event.target.value)} placeholder="Qty" />
            <select value={item.unit} onChange={(event) => updateManualIngredient(index, "unit", event.target.value)}><option value="oz">oz</option><option value="ml">ml</option><option value="tsp">tsp</option><option value="tbsp">tbsp</option><option value="dash">dash</option><option value="pc">pc</option><option value="cup">cup</option></select>
            <IngredientInput value={item.name} onChange={(value) => updateManualIngredient(index, "name", value)} listId={`manual-ingredient-${index}`} />
        ...
        <div className="toolbar"><button type="button" onClick={() => setManualIngredients((current) => [...current, { quantity: "1", unit: "oz", name: "" }])}>+ Ingredient</button></div>
        <h3>Instructions</h3>
        ...
        <div className="toolbar"><button className="primary" onClick={saveManual}>Save Recipe</button></div>
        {savedKey && <p className="success">Recipe saved successfully. <button className="link-button" onClick={() => openRecipe(savedKey)}>View Recipe</button></p>}
      </section>}

      {mode === "bulk" && collection && <section className="detail-card">
        <h2>{collection.count} recipes found</h2>
        <div className="toolbar"><button onClick={() => setSelected(collection.recipes.map((recipe) => recipe.url))}>Select All</button><button onClick={() => setSelected([])}>Clear</button><button className="primary" disabled={busy || selected.length === 0} onClick={() => void bulkImport()}>Import Selected ({selected.length})</button></div>
        <div className="result-list">{collection.recipes.map((item) => <label className="recipe-card" key={item.url}><span><input type="checkbox" checked={selected.includes(item.url)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, item.url] : current.filter((value) => value !== item.url))} /> {item.name}</span><small>{item.url}</small></label>)}</div>
      ...
      {mode === "single" && draft && <section className="detail-card">
        ...
        <label>Type <select value={draft.recipe_type} onChange={(event) => setDraft({ ...draft, recipe_type: event.target.value as "cocktail" | "mocktail" })}><option value="cocktail">Cocktail</option><option value="mocktail">Mocktail</option></select></label>
        <p><strong>Source:</strong> {draft.source_name}</p>
        {draft.warnings.length > 0 && <div className="notice">{draft.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}
        <h3>Ingredients</h3>
        <div className="import-ingredients">{draft.ingredients.map((item, index) => <div className="import-row" key={index}>
          ...
          <IngredientInput value={item.name} onChange={(value) => updateIngredient(index, "name", value)} listId={`import-ingredient-${index}`} />
        </div>)}</div>
        <h3>Instructions</h3>
